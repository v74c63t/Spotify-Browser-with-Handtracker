  ...
  runDetection(){
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            else if(openhands == 1 && !closedhands && !pointing && !pinching) this.detectedGesture = "Open Hand";
            
            if (closedhands > 1) this.detectedGesture = "Two Closed Hands";
            else if(closedhands == 1 && !openhands && !pointing && !pinching) this.detectedGesture = "Closed Hand";
            
            if (pointing > 1) this.detectedGesture = "Two Hands Pointing";
            else if(pointing == 1 && !closedhands && !openhands && !pinching) this.detectedGesture = "Hand Pointing";
